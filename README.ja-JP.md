...
- トレンドニュースのリアルタイム更新
- 検索ページ（人気検索/履歴/検索結果）
- 「興味なし」でアイテムを非表示（ローカルに永続化）
- 記事詳細（本文抽出はベストエフォート）と画像フルスクリーンビューア
...
# オプション: /api/detail のスクレイピング対象ドメインを制限（SSRF 対策）。
# カンマ区切り: 完全一致（example.com）またはサフィックス（*.example.com, .example.com）
DETAIL_ALLOWLIST=

# オプション: /api/detail の簡易レート制限とキャッシュ上限
# 0 にすると無効化。
